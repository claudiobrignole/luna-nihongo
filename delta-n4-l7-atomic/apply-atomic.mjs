#!/usr/bin/env node
/**
 * Applica il delta ATOMICO N4 Livello 7 ai repository (append/merge per-id).
...
const DELTA=di>=0?args[di+1]:"delta-n4-l7-atomic";
...
// levels
{
  const t=join(ROOT,"levels.json"); const cur=rj(t);
  const have=new Set(cur.levels.map(r=>r.level));
  let a=0,s=0; for(const r of rj(join(DELTA,"delta.levels.json")).levels){if(have.has(r.level)){s++;continue;}cur.levels.push(r);have.add(r.level);a++;}
  cur.levels.sort((x,y)=>x.level-y.level);
  log.push(`levels: +${a} (${s} già presenti) → ${cur.levels.length}`);
  if(!dry) writeFileSync(t,JSON.stringify(cur,null,2));
}
console.log((dry?"[DRY-RUN] ":"")+"delta atomico N4-L7:");
